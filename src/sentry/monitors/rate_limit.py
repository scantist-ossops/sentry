...
def get_project_monitor_quota(project_id: int, cache_bust=False) -> Tuple[int, int]:
    """
    Determines the rate-limit for monitor check-ins across a particular
    project.

    :return: A (window, limit) tuple
    """
    limit = None
    cache_key = f"project:{project_id}:monitor-env-count"
    ...
    if limit is None:
        monitor_count = MonitorEnvironment.objects.filter(monitor__project_id=project_id).count()
...
def update_monitor_quota(monitor_env: MonitorEnvironment):
    ...
    get_project_monitor_quota(project_id, cache_bust=True)
